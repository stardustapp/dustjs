<template>
  <aside id="left-menu" @transitionend="transitionend" @click="click">
    <nav id="navbar">
      <slot />
    </nav>
  </aside>
...
export default {
  ...
  mounted() {
    ...
    mc.on("panstart", (evt) => {
      ...
      if (!evt.velocityX) {
        console.log(
          "Sidenav refusing pan start event without X velocity",
          currentPan
        );
        return;
      }
      ...
      console.log(
        this.width,
        el.offsetLeft,
        Math.round(evt.center.x),
        this.width + el.offsetLeft - Math.round(evt.center.x)
      );
    ...
    mc.on("panend", (evt) => {
      ...
      if (currentPan != null) {
        ...
        adjustedOffset =
          offset +
          Math.round(Math.sqrt(evt.velocityX * 50) * (this.width / 10));
        nowOpen = adjustedOffset > -this.width / 2;
        targetX = nowOpen
          ? (el.classList.add("open"), 0)
          : (el.classList.remove("open"), -this.width);
