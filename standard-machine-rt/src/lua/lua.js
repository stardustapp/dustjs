...
class LuaMachine extends LuaContext {
  constructor(rootDevice, luaApi) {
    ...
    lauxlib.luaL_newmetatable(this.lua, "stardust/api");
      for (const callName in this.luaApi) {
  ...
  startThread() {
    ...
    const thread = new LuaThread(this, threadNum, this.luaApi);
...
class LuaThread extends LuaContext {
  constructor(machine, number, luaApi) {
    ...
    try {
      ...
      while (this.running) {
        ...
        case lua.LUA_YIELD:
          ...
          try {
            const impl = this.luaApi[callName];
            outputNum = await impl.call(this, L, T);
          } catch (err) {
            console.error('BUG: lua API crashed:', err.stack);
